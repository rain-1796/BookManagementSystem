...
namespace BOOKMS
{
    public partial class user2 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)//借出图书按钮
        {
...
#pragma warning disable CS8604 // 引用类型参数可能为 null。
            ...
            {
                string sql = $"insert into t_lend(uid,bid,bname,datetime) values({Data.UID},'{id}','{name}',now());update t_book set number = number - 1 where id = {id};insert into t_lend_all(uid,bid,bname,datetime) values({Data.UID},'{id}','{name}',now())";
                Dao dao = new Dao();
                if (dao.Execute(sql) > 2)//三张表各执行一条sql语句，至少会有三行受到影响
                {
                    MessageBox.Show($"用户{Data.UName}借出了图书：{name}");
                    Table();
                }
            }
        }
